...
extension ScannerPreviewView { // Расширение, чтобы держать Coordinator рядом с ScannerPreviewView.
    final class Coordinator: NSObject, AVCaptureMetadataOutputObjectsDelegate { // Coordinator получает события распознавания штрихкодов от AVFoundation.
        ...
        private func configureSessionIfNeeded() { // Настраивает AVCaptureSession один раз.
            ...
            configurePreferredFrameRate(for: device) // Если камера поддерживает 60 FPS, пробуем включить этот режим.
        ...
        private func configurePreferredFrameRate(for device: AVCaptureDevice) {
            let targetFrameRate: Double = 60
            let minimumFramePixelCount: Int32 = 1280 * 720
            let preferredFramePixelCount: Int32 = 1920 * 1080
            ...
            let supportedFormats = device.formats.filter { format in
                ...
                let hasEnoughPixels =
                    dimensions.width * dimensions.height >= minimumFramePixelCount
                let supportsTargetFrameRate =
                    format.videoSupportedFrameRateRanges.contains { range in
                        range.minFrameRate <= targetFrameRate
                            && targetFrameRate <= range.maxFrameRate
                    }

                return hasEnoughPixels && supportsTargetFrameRate
            }

            guard let format = supportedFormats.max(by: { lhs, rhs in
                ...
                let lhsPixelCount = lhsDimensions.width * lhsDimensions.height
                let rhsPixelCount = rhsDimensions.width * rhsDimensions.height
                let lhsDistance = abs(lhsPixelCount - preferredFramePixelCount)
                let rhsDistance = abs(rhsPixelCount - preferredFramePixelCount)

                return lhsDistance > rhsDistance
            }) else {
                print("Scanner 60 FPS format with 720p+ resolution was not found")
                return
            }
            ...
            do {
                ...
                print("Failed to configure scanner frame rate:", error)
            }
        }
